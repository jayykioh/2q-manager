...
-- Update checkout_order RPC to generate notifications
...
DECLARE
  ...
  v_product_id UUID;
  v_notification_msg TEXT := 'B?n có ðõn hàng m?i : ';
...
BEGIN
  ...
  LOOP
    ...
    v_notification_msg := v_notification_msg || v_product_record.name || ' (' || v_product_record.type || ') ,' || v_sale_price::TEXT || ' ; ';
  END LOOP;
  ...
  INSERT INTO public.transactions (
    ...
    p_store_id, 'income', 'sale', v_total, 'Thanh toán ðõn hàng ' || v_order_number, v_user_id, v_order_id
  );
  ...
  VALUES ('order', 'Ðõn hàng m?i ' || v_order_number, v_notification_msg)
...
-- Trigger for manual transactions (where order_id IS NULL)
...
BEGIN
  IF NEW.order_id IS NULL THEN
    INSERT INTO public.notifications (type, title, message)
    VALUES ('transaction', 
            CASE WHEN NEW.type = 'income' THEN 'Kho?n thu m?i' ELSE 'Kho?n chi m?i' END,
            COALESCE(NEW.description, 'Giao d?ch m?i') || ' - ' || NEW.amount::TEXT)
    RETURNING id INTO v_notification_id;
