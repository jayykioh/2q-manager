-- Remove invoice_jobs logic from checkout_order
...
-- Drop invoice_jobs table
DROP TABLE IF EXISTS public.invoice_jobs CASCADE;

-- Just to be fully clean, ensure outbox is dropped (already dropped in 0029, but safe to re-run)
DROP TABLE IF EXISTS public.notification_outbox CASCADE;

-- Unschedule any pg_cron jobs if they exist, specifically process-notification-outbox
DO $$
BEGIN
