...
**Một đường kẻ ngang 1px** `var(--color-ink)` xuyên qua toàn bộ layout như thread kết nối từng section. Grid system strict với 8px baseline. Số tiền và SKU luôn in Bebas Neue — tạo ra nhịp visual giống editorial spread hơn là app store template.
...
Image optimization: resize client-side thành thumbnail 400–600px và detail 1200–1600px,
                    WebP, target ≤ 800KB/ảnh; tạo blurhash/blurDataURL thực
...
CREATE TABLE product_images (
  ...
  blur_data   TEXT,                      -- blurhash hoặc blurDataURL đã tạo khi upload
...
-- Insert mặc định
-- Seed shift sau khi đã tạo default store; mọi shift phải có store_id.
...
3. Kiểm tra product còn `in_stock`, thuộc đúng store, không trùng trong payload; `sale_price >= 0`.
4. Tính `subtotal`, `discount`, `total`, `created_by`, `paid_at` và `order_number` ở database; không tin giá trị tổng từ client.
5. Insert order + items, đổi product thành `sold`, set `sold_at`, ghi `inventory_movements` loại `sale`.
6. Tạo notification/recipients, `notification_outbox` và `invoice_jobs` trong cùng transaction.
7. Commit rồi mới trả order. Bất kỳ bước nào lỗi thì rollback toàn bộ và trả mã lỗi ổn định (`PRODUCT_UNAVAILABLE`, `STORE_FORBIDDEN`, `INVALID_DISCOUNT`, ...).
...
**`create_product(...)`, `update_product_metadata(...)`, `set_product_reservation(...)`** là admin commands; không cho direct update `status`, `current_store_id`, `sold_at` hoặc `reserved_until` từ client.
...
**`cancel_order(p_order_id, p_reason)`** chỉ dành cho admin, lock order + products, chuyển order sang `cancelled`, ghi actor/time/reason và tạo return movements trong cùng transaction. Không update/delete order đã paid trực tiếp.
...
-- products/images/orders/items/attendance/notifications/recipients/inventory/jobs/outbox không có client write policy.
...
app/
  ...
  (authenticated)/
    ...
    app/                      — URL namespace /app/*, staff mobile layout
      layout.tsx
      pos/page.tsx
      products/
        page.tsx
...
Route group `(admin)` hoặc `(staff)` không tạo URL segment. Vì vậy các page trùng tên bên trong hai group sẽ conflict. Blueprint dùng segment thật `/admin/*` và `/app/*`; middleware/layout chỉ là defense-in-depth, quyền dữ liệu vẫn do RLS/RPC enforce.
...
- Tự động generate SKU dựa trên type được chọn
- Upload 1–4 ảnh: front, back, side, detail
- Fields: SKU (editable), Tên sản phẩm, Loại (dropdown), Tier (Premium/Standard), Giá gốc (auto-fill theo tier, editable), Chiều dài (mm), Khối lượng (g), Ghi chú
- Status mặc định: `Còn hàng`
- Resize + compress ảnh client-side trước khi upload
...
- [ ] Next.js project setup + TypeScript + Tailwind
- [ ] Supabase schema: stores, constraints, indexes, RLS, private helpers
- [ ] pgTAP permission matrix và concurrent checkout proof trước khi làm POS UI
- [ ] R2 bucket setup + presigned URL flow
- [ ] Design system: tokens, base components (Button, Input, Card, Badge)
- [ ] Auth flow: login, session, route protection, middleware
- [ ] PWA manifest + version-safe Service Worker cơ bản

### Phase 2 — Core Features (Tuần 3–4)
- [ ] Product management: list, add, edit, image upload
- [ ] Database SKU/order-number generator
- [ ] POS screen: browse → add to cart → checkout
- [ ] Transactional `checkout_order` với idempotency + row locks
- [ ] Async invoice job + private R2 + signed download
- [ ] Product status Realtime invalidation

### Phase 3 — Staff & Operations (Tuần 5–6)
- [ ] Attendance RPC: database time, server-computed hours/pay
- [ ] Admin: bảng chấm công, approve, tính lương
- [ ] Role-based UI (admin vs staff layout)
- [ ] Dashboard: doanh thu, stats
- [ ] Notification recipients + outbox + multi-device push subscriptions

### Phase 4 — Polish & Production (Tuần 7–8)
- [ ] Profile list performance; chỉ thêm virtual scroll nếu metrics yêu cầu
- [ ] R2 retention/orphan cleanup job + private invoice access audit
- [ ] Performance audit + Core Web Vitals
- [ ] PWA install prompt
- [ ] Error boundaries + loading states hoàn chỉnh
- [ ] Sentry/OTel, alert cho failed jobs, backup/restore drill
- [ ] Playwright critical flows + security headers + rate limiting
- [ ] Vercel production deploy
- [ ] Supabase upgrade Free → Pro
